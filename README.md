...
One important thing to mention with the implementation of this project is that rather than manage the shuffling / reshuffling of card shoes, I opted to define a standard deck of 52 cards (via, tuple) and decided to enable a random draw from this tuple (with replacement) every time a new card is needed within blackjack gameplay.
...
When initializing a hand, the 'random' module is pulling a random 2-card draw from our standard 52 card deck. Likewise, every time a hit method is called, a random card is pulled and added to the hand.

### Functions

#### Placing a bet

I developed a function that takes the user's current wallet value as an argument and will request / validate how much a user is willing and able to bet (see place_bet(), line 115 -> blackjack.py). In the case where a user has $100 and the table minimum is $25 ...
...
Remember I said we would revisit the ace value conundrum! Dynamically calculating the value of a hand was also a feat in itself, as I needed an easy way to account for the had / soft rule (see hand_calculator(), line 145 -> blackjack.py). It starts by taking a list of cards in the hand ...
...
The hand is iterated through, seperating the aces and accounting for all non-ace values. Only then, do we account for the aces ...
...
This function will replicate an entire single hand of blackjack from start to finish. Once a bet is placed and dedcuted from the user's wallet, two hands are initiated; one for the dealer and one for the player.
...
Then, we must check if the dealer has a high card, for the purpose of offering insurance (only if the user can afford the insurance premimum) ...
...
If insurance is taken, it will be deducted from the user's wallet. The next steps entail checking everyone's hand for blackjack, and either paying out the user either their insurance claim on dealer blackjack, paying out the winner on blackjack, or continuing the game.

Assuming the player made it, they will then be prompted to make decisions regarding their hand, until they chose to double down (appl. only on first move), stay, or reach 21+
...
In the case that the player busts, the game will end. Otherwise, we go to the showdown and make payouts accordingly, based on the decisions the user made throughout the game. All winnings (if any) are reapplied to the user's wallet (including prinicpal), and the wallet amount is returned. 

#### Account System

The account system is built utilizing three data elements (username, password, wallet value) that are comma seperated and stored as a line in a text file. To create an account, a user will input some phrase that will denoted their account and password, to protect their account. It's important to note that any commas are removed from what the user inputs, as it will make parsing the account info difficult.
...
Assuming the username is not already in use (see user_already_exists(), line 456 -> blackjack.py), a new line will be added to the account ledger (.txt) file. If that file does not yet exist, a new file will be created with the users account information.
...
As the user deposits money and/or inccurs winnings or losses from their gameplay, we will need to update their wallet value in the account ledger. To do that, we will take their username to find their line of account information, and overwrite their wallet value, via overwriting the .txt file with their new wallet amount (see update_wallet(), line 530 -> blackjack.py).
...
One important thing about this is that there is a variable, 'account' that is used to store the user's account info. If nobody is logged in, then the value of account will default to None. Otherwise, a users account information will be stored here, as a list. This vairable is used throughout most of the menu commands, as it is imparative to know who is logged in when performing most functions.
